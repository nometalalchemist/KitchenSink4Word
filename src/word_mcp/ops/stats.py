...
def word_count(pkg: DocxPackage, *, by_section: bool = True) -> dict:
    ...
    totals = {"words": 0, "characters": 0, "paragraphs": 0, "tables": 0}
    sections: list[dict] = []
    ...
    def add_counts(words: int, chars: int) -> None:
        totals["words"] += words
    ...
    for kind, idx, el in body_items(pkg):
        if kind == "paragraph":
            ...
            totals["paragraphs"] += 1
            add_counts(_count_words(text), len(text))
        else:
            totals["tables"] += 1
            ...
            add_counts(_count_words(table_text), len(table_text))
